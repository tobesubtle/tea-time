...
export async function GET(request: Request) {
  ...
  try {
    ...
    // 1. 크론 실행 결과 로그 기록 (성공)
    await cronLogRepo.saveLog({
      jobName: 'sync-gemini-models',
      status: 'success',
      message: `Gemini 모델 동기화 완료 (${updatedCount}개 업데이트)`,
      updatedCount,
      executionTimeMs: durationMs,
    });

    // 2. 1년 지난 오래된 크론 로그 자동 정리 (Auto Prune)
    const { deletedCount: prunedLogsCount } = await cronLogRepo.pruneLogsOlderThanOneYear();
    ...
    await cronLogRepo.saveLog({
      jobName: 'sync-gemini-models',
      status: 'failed',
      message: error.message || '알 수 없는 크론 실행 오류',
      updatedCount: 0,
      executionTimeMs: durationMs,
    });
    ...
    return NextResponse.json(
      {
        success: false,
        error: error.message || 'Cron execution error',
        executionTimeMs: durationMs,
      },
      { status: 500 }
    );
